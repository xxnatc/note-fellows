...
webDB.init = function() {
  ...
  try {
    if (openDatabase) {
      webDB.verbose(true);
      webDB.connect('notesDB', 'Notes Database', 5*1024*1024);
      webDB.setupTables();
    } else {
...
webDB.setupTables = function () {
  html5sql.process(
    'CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY, title VARCHAR(255) NOT NULL, user VARCHAR(255) NOT NULL, tags TEXT, content TEXT NOT NULL);',
    function() {
      // on success
